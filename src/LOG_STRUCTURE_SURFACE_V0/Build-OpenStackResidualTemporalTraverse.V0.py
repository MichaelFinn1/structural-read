...
SOURCE = Path(
    r"C:\Users\Admin\Desktop\StructuralRead_FirstUserTest\terrain\loghub_2k_panel\OpenStack\OpenStack_2k.log"
)

OUTDIR = Path("OpenStack_residual_temporal_001_out")
...
for window_size in WINDOWS:
    for start in range(0, len(lines), window_size):
        ...
        summary_rows.append({
            ...
            "stable_share": round(len(stable_idxs) / len(idxs), 4),
            "middle_share": round(len(middle_idxs) / len(idxs), 4),
            "residual_share": round(residual_count / len(idxs), 4),
...
out = OUTDIR / "OpenStack_residual_temporal_windows_v0.csv"
