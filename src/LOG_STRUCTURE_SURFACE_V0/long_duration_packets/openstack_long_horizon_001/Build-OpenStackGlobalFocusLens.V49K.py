...
OUT = BASE / "visualizations" / "openstack_global_focus_lens_v49k.html"
...
<head>
<meta charset="utf-8">
<title>OpenStack Global Focus Lens V49K</title>
<style>
...
.inspectionFrame {{
  ...
  background:rgba(24,138,58,0.18);
  z-index:30;
}}
...
<body>
<div id="wrap">
<h1>OpenStack Global Focus Lens V49K</h1>
...
function renderInspectionPreviewOnly() {{
  ...
  if (apertureStart !== inspectionStart || apertureEnd !== inspectionEnd) {{
    suffix =
      "\\n\\n... focal aperture shows " + apertureStart + "-" + apertureEnd +
      " inside green inspection span " + inspectionStart + "-" + inspectionEnd + ".";
  }}

  if (basinStart !== inspectionStart || basinEnd !== inspectionEnd) {{
    suffix +=
      "\\nSaving basin continues: " + basinStart + "-" + basinEnd + ".";
  }}
