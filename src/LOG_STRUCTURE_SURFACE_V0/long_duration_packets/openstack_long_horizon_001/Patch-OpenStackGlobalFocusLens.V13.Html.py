...
out = base / "openstack_global_focus_lens_v13.html"

text = src.read_text(encoding="utf-8")
text = text.replace("OpenStack Global Focus Lens V10", "OpenStack Global Focus Lens V13")

text = text.replace("</style>", """
.packetTray { border: 1px solid #999; padding: 10px; margin-top: 14px; }
...
const packetTray = document.createElement("div");
packetTray.className = "packetTray";
packetTray.innerHTML = `
  ...
  <div class="packetButtons">
    ...
    <button id="updateLoadedSticky">Update loaded sticky note</button>
...
document.getElementById("wrap").appendChild(packetTray);
...
const updateLoadedSticky = document.getElementById("updateLoadedSticky");
...
if (cards) {
  cards.addEventListener("click", evt => {
    ...
    if (!card) return;
    activeBookmarkIndex = Array.from(cards.children).indexOf(card);
  }, true);
}
...
addSelectedRaw.addEventListener("click", evt => {
  ...
  const rr = selectedRawRangeFallback();

  row.raw_start = rr.raw_start;
  row.raw_end = rr.raw_end;
  row.line_number = "";
  row.posture = "";
...
updateLoadedSticky.addEventListener("click", evt => {
  if (activeBookmarkIndex === null || !bookmarks || !bookmarks[activeBookmarkIndex]) {
    bookmarkPreview.textContent = "No loaded sticky note selected.";
    return;
  }
  ...
  bookmarks[activeBookmarkIndex].global_window = s.globalSize;
  bookmarks[activeBookmarkIndex].focus_window = s.focusSize;
  bookmarks[activeBookmarkIndex].focus_start = s.start;
  bookmarks[activeBookmarkIndex].focus_end = s.end;
  bookmarks[activeBookmarkIndex].focus_width = s.span;
  bookmarks[activeBookmarkIndex].raw_start = rr.raw_start;
  bookmarks[activeBookmarkIndex].raw_end = rr.raw_end;
  bookmarks[activeBookmarkIndex].operator_note = noteText.value;
  bookmarks[activeBookmarkIndex].unresolved_status = unresolvedText.value;
  bookmarks[activeBookmarkIndex].updated_local = new Date().toISOString();

  renderCards();

  bookmarkPreview.textContent =
    "Updated loaded sticky note.\n\n" + JSON.stringify(bookmarks[activeBookmarkIndex], null, 2);
