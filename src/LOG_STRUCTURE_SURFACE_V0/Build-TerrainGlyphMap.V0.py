...
parts.append(f'<text x="{width/2}" y="32" text-anchor="middle" font-size="22">Six-Terrain Glyph Map V0</text>')
...
for r in rows:
    ...
    parts.append(f'<text x="{x + radius + 8:.1f}" y="{y + 4:.1f}" font-size="14">{name}</text>')

parts.append('</svg>')

out = OUT / "terrain_glyph_map_v0.svg"
