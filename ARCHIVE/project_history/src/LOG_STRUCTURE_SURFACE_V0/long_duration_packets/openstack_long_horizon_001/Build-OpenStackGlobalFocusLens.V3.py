...
OUT = BASE / "visualizations" / "openstack_global_focus_lens_v3.html"
...
<head>
<meta charset="utf-8">
<title>OpenStack Global Focus Lens V3</title>
<style>
...
<body>
<div id="wrap">
<h1>OpenStack Global Focus Lens V3</h1>
...
<div class="note">
Use the upper bar as the map. Drag across it to select a focus region, or use the focus center and focus width sliders. Global and focus windows are independent.
</div>
</div>
...
function renderBar(bar, size, start, end, fullScale) {{
  clearBar(bar);
  const rows = data.rows.filter(r => parseInt(r.window_size) === size && r.line_end >= start && r.line_start <= end);
