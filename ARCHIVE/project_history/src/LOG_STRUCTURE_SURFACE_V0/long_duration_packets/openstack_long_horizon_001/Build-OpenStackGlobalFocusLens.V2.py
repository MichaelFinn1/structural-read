...
OUT = BASE / "visualizations" / "openstack_global_focus_lens_v2.html"
...
<head>
<meta charset="utf-8">
<title>OpenStack Global Focus Lens V2</title>
<style>
body {{
...
<body>
<div id="wrap">
<h1>OpenStack Global Focus Lens V2</h1>

<div class="controls">
  <div class="box">
    <div>Global constitution</div>
...
<div class="note">
Use the upper bar as the map. Drag across it to select a focus region, or use the focus center and focus width sliders. Global and focus constitutions are independent, so the map can stay broad while the lens uses a finer reread.
</div>
</div>
...
function lineFromMouse(evt) {{
  const rect = globalBar.getBoundingClientRect();
  const x = clamp(evt.clientX - rect.left, 0, rect.width);
  return Math.round(1 + (x / rect.width) * (data.max_line - 1));
}}
...
function render() {{
  ...
  globalSizeLabel.innerHTML = "global window: " + s.globalSize;
  focusSizeLabel.innerHTML = "focus window: " + s.focusSize;
...
globalBar.addEventListener("mousedown", evt => {{
  dragging = true;
  dragStartLine = lineFromMouse(evt);
...
globalBar.addEventListener("mousemove", evt => {{
  ...
  const currentLine = lineFromMouse(evt);
...
globalBar.addEventListener("mouseup", evt => {{
  ...
  const endLine = lineFromMouse(evt);
...
globalBar.addEventListener("mouseleave", evt => {{
  if (dragging) {{
    dragging = false;
    if (dragBox) {{
      dragBox.remove();
