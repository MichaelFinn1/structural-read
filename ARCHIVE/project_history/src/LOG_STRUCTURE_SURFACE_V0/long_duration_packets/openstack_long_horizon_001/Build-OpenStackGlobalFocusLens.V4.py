...
OUT = BASE / "visualizations" / "openstack_global_focus_lens_v4.html"
...
<head>
<meta charset="utf-8">
<title>OpenStack Global Focus Lens V4</title>
<style>
...
<body>
<div id="wrap">
<h1>OpenStack Global Focus Lens V4</h1>
...
<pre id="linePreview">Click inside the focus lens to preview raw source lines.</pre>

<div class="note">
Use the upper bar as the map. Click or drag on it to choose a region. Mouse wheel over the global map changes the focus window size. Click inside the focus lens to preview raw lines.
</div>
</div>
...
function previewLines(centerLine) {{
  ...
  const end = Math.min(data.max_line, centerLine + radius);
  let out = [];
...
focusBar.addEventListener("click", evt => {{
  const s = state();
  const line = lineFromMouse(evt, focusBar, s.start, s.end);
  previewLines(line);
}});
