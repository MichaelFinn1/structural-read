...
SRC = Path("src/LOG_STRUCTURE_SURFACE_V0/movement_packets/movement_topology_v1_001/measured_netsparker_window_dial_dense_v1/traversal_windows_v0.csv")
OUT = Path("src/LOG_STRUCTURE_SURFACE_V0/movement_packets/movement_topology_v1_001/movement_visualizations/local_ecology_window_dial_dense_v1.html")
...
<head>
<meta charset="utf-8">
<title>Local Ecology Window Dial Dense V1</title>
<style>
...
.seg {{ height: 100%; }}
.stable {{ background: rgba(245,245,245,0.95); }}
.middle {{ background: rgba(120,120,120,0.38); }}
.residual {{ background: rgba(30,30,30,0.62); }}
.windowBoundary {{
  position: absolute;
  top: 0;
  bottom: 0;
...
.note {{ margin-top: 25px; font-size: 16px; line-height: 1.45; }}
</style>
</head>
<body>
<div id="wrap">
<h1>Local Ecology Window Dial Dense V1</h1>
<div id="label"></div>
...
<div class="note">
Read question: where does local ecology fragment, recompose, smooth, or remain coherent as constitution changes?
<br>
Boundary: dense sampling is not truth extraction. It only reduces jumpiness between declared reread constitutions.
</div>
</div>
...
function stanceName(size) {{
  ...
  if (size >= 50) return "fragmentation threshold";
  return "sub-fragment probe";
}}
